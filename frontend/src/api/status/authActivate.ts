...
enum AuthErrorType {
    TYPE_UNDEFINED,
    InvalidUser, // 341
    USER_ALREADY_LOGGED_IN, // 400
    ServerError, // 500 서버 내부 문제
}

// activate
function validateAuthActivateResponseStatus(response: Response) {
    if (response.ok) {
        console.log("[DEV] 활성화 완료, 결과로 QR 코드 이미지를 전달받습니다.");
        return ; // do nothing
    } else if (response.status === 400) {
      throw new AuthError(AuthErrorType.USER_ALREADY_LOGGED_IN, "[DEV] 이미 접속된 유저입니다.");
...
export async function fetchAndHandleAuthActivateError(url: string, requestObject?: RequestInit) {
  ...
  try 
  {
    ...
    validateAuthActivateResponseStatus(response); // 여기서 status code throw.
    return (response);
  } 
  catch (err: unknown) 
  {
    if (err instanceof AuthError) {
      switch(err.errorStatus) {
        case AuthErrorType.TYPE_UNDEFINED:
          //...
          break;
        case AuthErrorType.InvalidUser:
          //...
          break; 
        case AuthErrorType.USER_ALREADY_LOGGED_IN:
          //...
          break; 
        default:
          // ... 여기는 말이 안됨. Assert 넣기.
