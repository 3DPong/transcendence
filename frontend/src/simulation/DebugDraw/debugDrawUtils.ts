import type { Helpers } from "@/simulation/Helpers";
...
export class CanvasDebugDraw {
  ...
  drawPoint = (vec_p: number, sizeMetres: number, color_p: number): void => {
    ...
    this.context.fillRect(vert.get_x() * this.aspectRatio - sizePixels/2, vert.get_y() * this.aspectRatio - sizePixels/2, sizePixels, sizePixels);
  }
  ...
  drawSegment = (vert1_p: number, vert2_p: number): void => {
    ...
    this.context.moveTo(vert1V.get_x() * this.aspectRatio, vert1V.get_y() * this.aspectRatio);
    this.context.lineTo(vert2V.get_x() * this.aspectRatio, vert2V.get_y() * this.aspectRatio);
    this.context.stroke();
  }
  ...
  drawPolygon = (vertices: number, vertexCount: number, fill: boolean): void => {
    ...
    for(let tmpI=0; tmpI < vertexCount; tmpI++) {
      ...
      if (tmpI === 0) {
        this.context.moveTo(vert.get_x() * this.aspectRatio, vert.get_y() * this.aspectRatio);
      } else {
        this.context.lineTo(vert.get_x() * this.aspectRatio, vert.get_y() * this.aspectRatio);
      }
    }
  ...
  drawCircle = (center_p: number, radius: number, axis_p: number, fill: boolean): void => {
    ...
    this.context.beginPath();
    this.context.arc(centerV.get_x() * this.aspectRatio,centerV.get_y() * this.aspectRatio, radius * this.aspectRatio, 0, 2 * Math.PI, false);
    if (fill) {
      this.context.fill();
    }
    this.context.stroke();
    ...
    if (fill) {
      ...
      this.context.beginPath();
      this.context.moveTo(centerV.get_x() * this.aspectRatio, centerV.get_y() * this.aspectRatio);
      this.context.lineTo(vert2V.get_x() * this.aspectRatio, vert2V.get_y() * this.aspectRatio);
      this.context.stroke();
    }
  }
  ...
  drawTransform = (transform_p: number): void => {
    ...
    this.context.save();
    this.context.translate(pos.get_x() * this.aspectRatio, pos.get_y() * this.aspectRatio);
