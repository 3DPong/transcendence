...
const AvatarBadge: FC<AvatarBadgeProps> = ({ user, children, isNone }) => {
  if (isNone) return <Box sx={{ paddingLeft: "6px", paddingRight: "2px", zIndex: 2 }}>{children}</Box>;
  else
    return (
      <Badge
...
const AvatarSet: FC<AvatarSetProps> = ({ user, scrollY }) => {
  ...
  return (
    <AvatarBadge user={user} isNone={isNone}>
      <Avatar src={user.profile} alt={user.nickname} className="cursor-pointer" onClick={handleAvatarClick} />
      {Boolean(anchorEl) && (
        <AvatarPopper
          anchorEl={anchorEl}
          handleClose={() => {
