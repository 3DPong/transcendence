...
export class GameGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  gameMatchJoin(@MessageBody() matchJoinData: MatchJoinData, @ConnectedSocket() client: Socket) {
  ...
  chatMatchJoin(@MessageBody() matchJoinData: ChatJoinData, @ConnectedSocket() client: Socket) {
    ...
    if (gameManager === undefined) {
      ...
      this.server
        .to(client.id)
        .emit('onGameInvite', new InviteData(gameManager.gameId, matchJoinData.channelId, gameManager.gameType));
      this.logger.log(`gameCreate ${gameManager.gameId}`);
    } else {
      gameManager.createPlayer(client.id, client.data.userId);
      this.gameService.socketJoinRoom(client, gameManager.gameId);
  ...
  matchExit(@ConnectedSocket() client: Socket) {
    this.gameService.gameExit(this.gameRooms, client);
  }

  //observe
  @SubscribeMessage('observeJoin')
  observerJoin(@MessageBody() observeData: ObserveData, @ConnectedSocket() client: Socket) {
  ...
  keyInput(@MessageBody() inputData: InputData, @ConnectedSocket() client: Socket) {
  ...
  matchStart(@ConnectedSocket() client: Socket) {
